...
def get_model(config) :
    ...
    if type_attention == "categorical" :
        attention_mlp = get_measurement_attention_mlp(batch_size=batch_size, num_input_features=num_category_bmi + num_category_height)
        ...
        # ✅ 이 부분
        attention_mlp = get_measurement_attention_mlp(batch_size=batch_size, num_input_features=2)  # ✅ 원코드
    ...
    if type_backbone == "blazepose" :
        if type_attention != "none" :
            ...
            # blazepose_model = BlazePose(batch_size=batch_size, input_shape=input_shape, num_keypoints=31, num_seg_channels=14, attention_model=attention_mlp)
            blazepose_model = BlazePose(batch_size=batch_size, input_shape=input_shape, num_keypoints=31, num_seg_channels=2, attention_model=attention_mlp)
        else :
